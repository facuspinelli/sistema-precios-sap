...
function CommercialAnalysis({ analisis, onExportWord, generandoWord }: { analisis: ResultadoComercial; onExportWord: () => void; generandoWord: boolean }) {
  ...
  return <section className="analysisPanel" id="analisis-comercial">
    ...
    <div className="analysisGrid">
      ...
      <div className="panel"><div className="panelHeader"><div><h3>Oportunidades</h3><p>Umbral de señal: ±{UMBRAL_OPORTUNIDAD}%. El % mostrado es el porcentaje real de diferencia; el umbral solo determina cuándo marcar una oportunidad.</p></div></div>{topOpps.length ? topOpps.map((o, i) => <div className="opportunity" key={`${o.material}-${o.razonSocial}-${i}`}><div className={o.tipo === "PRECIO_ALTO" ? "oppTag high" : "oppTag low"}>{o.tipo === "PRECIO_ALTO" ? "PRECIO ALTO" : "PRECIO BAJO"}</div><div><strong>{o.razonSocial}</strong><span>{o.material} · {o.familia}</span><small>{pct(o.porcentaje)} vs referencia · $ {money(o.importe)} vs $ {money(o.referencia)} · {o.veces.toFixed(1)}x</small></div></div>) : <div className="noResults"><strong>No se detectaron oportunidades fuertes</strong>Con el universo seleccionado no aparecen diferencias superiores al ±{UMBRAL_OPORTUNIDAD}%.</div>}</div>
    </div>

    <div className="panel"><div className="panelHeader"><div><h3>Comparación por material y cliente</h3><p>El porcentaje es la diferencia real contra la referencia de los otros clientes del mismo material. Ejemplo: $10 vs $5 = +100%; $5 vs $10 = -50%.</p></div></div><div className="tableScroll"><table><thead><tr><th>Familia</th><th>Material</th><th>Cliente</th><th>Promedio</th><th>Diferencia</th><th>Vs otros</th><th>Posición</th></tr></thead><tbody>{analisis.gruposMaterial.slice(0, 100).flatMap(g => g.clientes.map(c => <tr key={`${g.material}-${c.cliente}`}><td>{g.familia}</td><td><strong>{g.material}</strong><small>{g.descripcion}</small></td><td>{c.razonSocial}</td><td>$ {money(c.importePromedio)}</td><td>{c.diferenciaVsPromedioOtros >= 0 ? "+" : "-"}$ {money(Math.abs(c.diferenciaVsPromedioOtros))}</td><td className={c.porcentajeVsPromedioOtros >= 0 ? "estadoAlerta" : "estadoOk"}>{pct(c.porcentajeVsPromedioOtros)} <small>{c.vecesVsPromedioOtros ? `${c.vecesVsPromedioOtros.toFixed(1)}x` : ""}</small></td><td>#{c.posicion}</td></tr>))}</tbody></table></div></div>

    <div className="panel familyClientPanel"><div className="panelHeader"><div><h3>Comparación por familia y cliente</h3><p>El promedio de familia es descriptivo. Cuando hay materiales compartidos, el % compara el promedio del cliente contra la referencia de los otros clientes dentro de esa misma familia.</p></div></div><div className="tableScroll"><table><thead><tr><th>Familia</th><th>Cliente</th><th>Registros</th><th>Materiales</th><th>Promedio</th><th>Referencia otros</th><th>Diferencia</th><th>Vs otros</th><th>Comparables</th></tr></thead><tbody>{analisis.gruposFamiliaCliente.slice(0, 100).map((x, i) => <tr key={`${x.familia}-${x.cliente}-${i}`}><td>{x.familia}</td><td>{x.cliente}</td><td>{x.registros}</td><td>{x.materiales}</td><td>$ {money(x.importePromedio)}</td><td>{x.materialesComparables ? `$ ${money(x.referenciaOtros)}` : "-"}</td><td>{x.materialesComparables ? `${x.diferenciaVsOtros >= 0 ? "+" : "-"}$ ${money(Math.abs(x.diferenciaVsOtros))}` : "-"}</td><td className={x.materialesComparables ? (x.porcentajeVsOtros >= 0 ? "estadoAlerta" : "estadoOk") : ""}>{x.materialesComparables ? `${pct(x.porcentajeVsOtros)} · ${x.vecesVsOtros.toFixed(1)}x` : "Sin comparación"}</td><td>{x.materialesComparables}</td></tr>)}</tbody></table></div></div>
  </section>;
}
